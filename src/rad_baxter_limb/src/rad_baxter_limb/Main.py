...
class RobotArm:
    ...
    def acquire_targ(self):
        ...
        hsv_shape = hsv.shape
        print(('HSV Value:', hsv[int(hsv_shape[0]/2), int(hsv_shape[1]/2),:]))
    ...
    def init_spots(self):
        input("Move to the safe position and press enter.")
        self.safe_pos = self.limb.get_joint_angles()
        input("Move to the collection position and press enter.")
        self.collection_pos = self.limb.get_joint_angles()
        input("Move to the bucket 1 position and press enter.")
        self.bucket1_pos = self.limb.get_joint_angles()
        input("Move to the bucket 2 position and press enter.")
        self.bucket2_pos = self.limb.get_joint_angles()
        input("Move to the bucket 3 position and press enter.")
    ...
    def movin(self, target):
        ...
        step = 1
        error = 1e6
        while step < 2500 and error >=1e-4:
            diff = self.limb.get_joint_angles()-target
    ...
    def main(self, args):
        ...
        if not args.get("spots", False):
            ...
            self.init_spots()
            spots = np.array([self.safe_pos, self.bucket1_pos, self.bucket2_pos, self.bucket3_pos, self.collection_pos])#self.z_ball])
            ...
            self.collection_pos = spots[4]
            # self.z_ball = spots[4]

        self.init_gripper_right()

        # Move to safe spot and then to collection
        # self.movin(self.safe_pos)
        self.movin(self.collection_pos)
